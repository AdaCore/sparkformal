...
Require Export checks_optimization_soundness_completeness_util.
...
(** * Copy_out_args Checks Optimization Completeness *)
Lemma copy_out_args_checks_optimization_completeness: forall params' st st' s f s' params args args' args'',
  toArgsRT st params args args' ->
  toSymTabRT st st' ->
  toParamSpecsRT params params' ->
  well_typed_symbol_table st' ->
...
- destruct args', args'', params, args;
  ...
  + apply CopyOut_Mode_Out_nRTE_X with (v:=v); auto.
    ...
    apply_store_update_optimization_completeness; auto.
  ...
  + apply_store_update_optimization_completeness; apply_store_update_ex_cks_stripped.
...
Ltac apply_copy_out_args_checks_optimization_completeness :=
  match goal with
  | [H1: toArgsRT ?st ?params ?args ?args',
     ...
     H11: copyOutRT ?st' ?s ?f ?params' ?args' ?s' |- _ ] =>
      specialize (copy_out_args_checks_optimization_completeness _ _ _ _ _ _ _ _ _ _ H1 H2 H3 H4 H5 H6 H7 H8 H9 H10 H11);
      let HZ := fresh "HZ" in intro HZ
  end.

(** * Copy_out_args Checks Optimization Soundness *)
Lemma copy_out_args_checks_optimization_soundness: forall st params args args' params' st' s f s' args'',
  toArgsRT st params args args' ->
  toSymTabRT st st' ->
  toParamSpecsRT params params' ->
  well_typed_symbol_table st' ->
...
-(* C2_Flagged_Args_Out_RangeCheck *)
 ...
 apply store_update_optimization_soundness 
 ...
 assert(HA3: storeUpdateRT st' s (update_exterior_checks_name n' (name_exterior_checks nRT)) (Int x) (OK s'0)).
   apply_store_update_ex_cks_added; auto.
 apply_store_update_optimization_soundness.
...
(** * Statement Checks Optimization Completeness *)
Lemma statement_checks_optimization_completeness: forall st st' s s' c c' c'',
  evalStmtRT st' s c' s' ->
    toSymTabRT st st' ->
...
- inversion H2; subst;
  ...
  + apply EvalAssignRT_RTE; auto.
    apply_expression_checks_optimization_completeness; auto.
  + apply EvalAssignRT_RTE; auto.
    apply_expression_checks_optimization_completeness.
...
- (* 14. Eval_S_Proc_RTE_Args_X *)
  ...
  specialize (symbol_table_procedure_rel _ _ _ _ _ H1 H13); intro HZ1.
    ...
    inversion HZ2; subst.
  apply_copy_in_args_checks_optimization_completeness; auto.
...
- (* 17. Eval_S_Proc_X *)
  ...
  + simpl in *.
    ...
    apply copy_out_args_checks_optimization_completeness with (st:=st0) (params:=params) (args:=args0) (args':=args); auto; simpl in *.
...
Ltac apply_statement_checks_optimization_completeness :=
  match goal with
  | [H1: evalStmtRT ?st' ?s ?c' ?s',
     ...
     H5: toStmtRT ?st ?c ?c',
     H6: optStmt ?st' ?c' ?c'' |- _] =>
      specialize (statement_checks_optimization_completeness _ _ _ _ _ _ _ H1 H2 H3 H4 H5 H6);
      let HZ := fresh "HZ" in intros HZ
  end.

(** * Statement Checks Optimization Soundness *)
...
- (* 17. EvalCallRT *)
  ...
  + simpl in *.
    assert(HA: well_typed_value_in_store st (snd (STACK.newFrame n))).
      smack; constructor.
    apply_copy_in_args_checks_optimization_soundness.
    apply_copy_in_preserve_typed_store.
    apply_eval_declaration_preserve_typed_store.
    assert(HA1: well_typed_value_in_stack st (f1 :: s1)).
      apply_well_typed_store_stack_merge'; auto.
    ...
    apply_well_typed_store_split'.
    assert(HA2: well_typed_value_in_stack st (intact_s ++ s3)). apply well_typed_stacks_merge'; auto.
    apply copy_out_args_checks_optimization_soundness with (st:=st0) (params:=params) (args:=args1) (args'':=args); auto; simpl in *.
- (* 18. EvalSeqRT_RTE *)
  inversion H2; subst;
  inversion H3; subst;
  inversion H4; subst;
...
Ltac apply_declaration_checks_optimization_soundness :=
  match goal with
...
(** * Declaration Checks Optimization Completeness *)
Lemma declaration_checks_optimization_completeness: forall st st' s f f' d d' d'',
...
Ltac apply_declaration_checks_optimization_completeness :=
  match goal with
  | [H1: evalDeclRT ?st' ?s ?f ?d' ?f',
     ...
     H6: toDeclRT ?st ?d ?d',
     H7: optDecl ?st' ?d' ?d'' |- _] =>
      specialize (declaration_checks_optimization_completeness _ _ _ _ _ _ _ _ H1 H2 H3 H4 H5 H6 H7);
      let HZ := fresh "HZ" in intros HZ
  end.
...
Proof.
  intros;
  split; intro;
  [eapply expression_checks_optimization_completeness; smack |
   eapply expression_checks_optimization_soundness; smack
  ].
Qed.
...
Proof.
  intros;
  split; intro;
  [eapply statement_checks_optimization_completeness; smack |
   eapply statement_checks_optimization_soundness; smack
  ].
Qed.
...
Proof.
  intros;
  split; intro;
  [eapply declaration_checks_optimization_completeness; smack |
   eapply declaration_checks_optimization_soundness; smack
  ].
Qed.
