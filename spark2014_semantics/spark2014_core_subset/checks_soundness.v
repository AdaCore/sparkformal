...
Proof.
  intros;
  match goal with
  | [H: do_run_time_check_on_binop _ _ _ _ |- _] => inversion H; clear H; crush;eauto
  end;
  match goal with
  | [H: do_overflow_check ?v ?v' |- _] => destruct v'
  end;
  match goal with
  | [H: do_overflow_check ?v ?v' |- _] => inversion H; crush;eauto
  end;
  repeat progress match goal with
    | [|- do_flagged_checks_on_binop _ _ _ _ _ ] => econstructor; crush;eauto
    | [|- do_flagged_check_on_binop  _ _ _ _ _ ] => econstructor; crush;eauto
  end.
Qed.
...
Proof.
  ...
  | [H1: do_division_check _ _ _, H2: do_overflow_check _ _ |- _] => 
      inversion H1; crush;eauto; inversion H2; crush;eauto
  end.
Qed.
...
Proof.
  ...
  match goal with
    | [H: do_run_time_check_on_binop _ _ _ _ |- _] => 
        inversion H; clear H; crush;eauto
  end;
  constructor; auto.
Qed.
...
Proof.
  ...
  destruct v';
  repeat progress match goal with
    | [H: do_run_time_check_on_unop _ _ _ |- _] => inversion H; clear H; crush;eauto
    | [|- do_flagged_checks_on_unop _ _ _ _] => econstructor; crush;eauto
    | [|- do_flagged_check_on_unop  _ _ _ _] => econstructor; crush;eauto 
    end;
  match goal with
...
Proof.
  ...
  repeat progress match goal with
  | [H: do_run_time_check_on_unop _ _ _ |- _] => inversion H; clear H; crush;eauto
  | [|- do_flagged_checks_on_unop _ _ _ _] => constructor; crush;eauto
  end.  
Qed.
...
Proof.
  ...
  - (* 1 *)
  match goal with
  | [H: compile2_flagged_exp _ _ _ |- _] => inversion H; clear H; crush;eauto
  end;
  match goal with
  | [H: eval_expr _ _ _ _ |- _] => inversion H; clear H; crush;eauto
  end;
  constructor; auto;
  repeat progress  match goal with
    | [H: eval_literal _ _ |- _] => inversion H; clear H; crush;eauto
    | [H: do_overflow_check _ _ |- _] => inversion H; clear H; crush;eauto
    end;
  repeat progress match goal with
    | [|- eval_literal_x _ _ _] => constructor
    | [|- do_overflow_checks _ _ _] => constructor
    | [|- do_overflow_check _ _] => constructor; crush;eauto
    end;
  repeat progress match goal with
  ...
  | [H1: forall (e' : expression_x) (st : symboltable) 
     ...
     H4: compile2_flagged_symbol_table _ _ |- _] =>
      specialize (H1 _ _ _ _ _ H2 H3 H4); crush;eauto
  end;
  match goal with
  ...
  | [H: do_run_time_check_on_binop Divide _ _ _ |- _] => 
      apply binop_overflow_division_check_soundness; crush;eauto
  | _ => idtac
  end;
  match goal with
  | [H1: ?op = Plus -> False,
     H2: do_run_time_check_on_binop ?op _ _ _ |- _] =>
      apply binop_no_check_soundness; crush;eauto
  end.
  - (* 4 *)
  match goal with
  | [H: compile2_flagged_exp _ _ _ |- _] => inversion H; clear H; crush;eauto
  ...
  | [H1: ?op = Unary_Minus -> False,
     H2: eval_expr _ _ ?e (Normal _) |- _] => 
      apply Eval_E_Unary_Operation_X with (v := v0); auto;
      apply unop_no_check_soundness; crush;eauto
  end.
  - (* 5 *)
  match goal with
  | [H: compile2_flagged_name _ _ _ |- _] => inversion H; clear H; crush;eauto
  end;
  match goal with
  | [H: eval_name _ _ _ _ |- _] => inversion H; clear H; crush;eauto
  end;
  constructor; auto.
  - (* 6 *)
  match goal with
  ...
  + apply Eval_E_Indexed_Component_Range_RTE_X with (a:=a0) (i:=i) (t:=t) (l:=l) (u:=u); crush;eauto.
    apply eval_exp_ex_cks_added; auto.
  ...
  + apply Eval_E_Indexed_Component_X with (a:=a0) (i:=i) (t:=t) (l:=l) (u:=u); crush;eauto.
    apply eval_exp_ex_cks_added; auto.
    rewrite <- (name_ast_num_eq _ _ _ H10);
    apply symbol_table_exp_type_rel with (st := st); auto.
    apply index_range_rel with (st := st); auto.
...
Proof.
  ...
  | [H1: eval_expr _ ?s ?e ?v,
     ...
     H3: compile2_flagged_symbol_table _ _ |- _] => specialize (expression_checks_soundness _ _ _ _ _ _ H1 H2 H3); crush;eauto
  | _ => idtac
  end;
  match goal with
  | [H1: forall (n' : name_x) (st : symboltable) 
...
Proof.
  ...
  induction H; intros;
  match goal with
  | [H: compile2_flagged_declaration _ _ _ |- _] => inversion H; clear H; crush;eauto
  end;
  match goal with
  | [H: compile2_flagged_object_declaration _ _ _ |- _] => inversion H; clear H; crush;eauto
  | _ => idtac
  end;
  match goal with
  ...
  | [H1: eval_expr _ ?s ?e ?v,
     ...
     H3: compile2_flagged_symbol_table _ _ |- _] => 
      specialize (expression_checks_soundness _ _ _ _ _ _ H1 H2 H3); crush;eauto
  | _ => idtac
  end;
  [ apply Eval_Decl_Null_X |
    apply Eval_Decl_Type_X |
    apply Eval_Decl_Var_None_X; auto |
    apply Eval_Decl_Var_eRTE_X with (e:=e_flagged); crush;eauto |
    apply Eval_Decl_Var_eRTE_X with (e:=(update_exterior_checks_exp e_flagged (Do_Range_Check :: nil))); auto |
    apply Eval_Decl_Var_NoCheck_X with (e:=e_flagged); crush;eauto |
    apply Eval_Decl_Var_Range_RTE_X with (e:=(update_exterior_checks_exp e_flagged (Do_Range_Check :: nil))) 
                                           (v:=v) (l:=l) (u:=u); crush;eauto |
    apply Eval_Decl_Var_X with (e:=(update_exterior_checks_exp e_flagged (Do_Range_Check :: nil))) (v:=v) (l:=l) (u:=u); crush;eauto |
  ...
  | [ |- context[name_exterior_checks (update_exterior_checks_name _ _)]] => rewrite name_updated_exterior_checks; crush;eauto
  | [ |- context[exp_exterior_checks (update_exterior_checks_exp _ _)]] => rewrite exp_updated_exterior_checks; auto
  | [H: context[name_exterior_checks (update_exterior_checks_name _ _)] |- False] => rewrite name_updated_exterior_checks in H; crush;eauto
  | [H1: compile2_flagged_symbol_table ?st ?st',
     H2: extract_subtype_range ?st ?t (Range _ _) |- 
     extract_subtype_range_x ?st' ?t (Range_X _ _)] =>
       specialize (subtype_range_rel _ _ _ _ _ H1 H2); crush;eauto
  | _ => idtac
  end;
  match goal with
  | [H: compile2_flagged_exp _ _ _ |- _] => specialize (exp_exterior_checks_beq_nil _ _ _ H); crush;eauto
  end.
Qed.
...
- match goal with
  | [H: storeUpdate _ _ _ _ _ |- _] => inversion H; clear H; crush;eauto
  end;
  match goal with
  | [H1: eval_name _ ?s ?n ?v,
     H2: compile2_flagged_name _ ?n ?n',
     H3: compile2_flagged_symbol_table _ _ |- _] => specialize (name_checks_soundness _ _ _ _ _ _ H1 H2 H3); crush;eauto
  | _ => idtac
  end;
  match goal with
  | [H1: eval_expr _ ?s ?e ?v,
  ...
  | [H1: compile2_flagged_symbol_table ?st ?st', (**)
     ...
     extract_array_index_range_x ?st' ?t (Range_X _ _)] =>
       specialize (index_range_rel _ _ _ _ _ H1 H2); crush;eauto
  | _ => idtac
  end;
  match goal with
...
- destruct args, args', params';
  ...
  | [H1: is_range_constrainted_type (parameter_subtype_mark ?x) = false,
     H2: extract_subtype_range st (parameter_subtype_mark ?x) (Range _ _) |- _] => 
      destruct (parameter_subtype_mark x); inversion H2; crush;eauto
  | _ => idtac
  end;
  match goal with
  | [H1: eval_expr _ ?s ?e ?v,
  ...
  [ apply Copy_In_Mode_In_eRTE_X; crush;eauto |
    apply Copy_In_Mode_In_NoRangeCheck_X with (v:=v) (f':=(STACK.push f (parameter_name a) v)); crush;eauto |
    apply Copy_In_Mode_In_eRTE_X; crush;eauto |
    apply Copy_In_Mode_In_Range_RTE_X with (v:=v) (l:=l) (u:=u); crush;eauto |
    apply Copy_In_Mode_In_Range_X with (v:=v) (l:=l) (u:=u) (f':=(STACK.push f (parameter_name a) (Int v))); crush;eauto |
    apply Copy_In_Mode_Out_X with (f':=(STACK.push f (parameter_name a) Undefined)); crush;eauto |
    apply Copy_In_Mode_Out_X with (f':=(STACK.push f (parameter_name a) Undefined)); crush;eauto |
    apply Copy_In_Mode_InOut_eRTE_x; crush;eauto |
    apply Copy_In_Mode_InOut_NoRange_X with (v:=v) (f':=(STACK.push f (parameter_name a) v)); crush;eauto |
    apply Copy_In_Mode_InOut_eRTE_x; crush;eauto |
    apply Copy_In_Mode_InOut_Range_RTE_X with (v:=v) (l:=l) (u:=u); crush;eauto |
    apply Copy_In_Mode_InOut_Range_X with (v:=v) (l:=l) (u:=u) (f':=(STACK.push f (parameter_name a) (Int v))); crush;eauto |
    apply Copy_In_Mode_InOut_eRTE_x; crush;eauto | 
    apply Copy_In_Mode_InOut_NoRange_X with (v:=v) (f':=(STACK.push f (parameter_name a) v)); crush;eauto |
    apply Copy_In_Mode_InOut_eRTE_x; crush;eauto |
    apply Copy_In_Mode_InOut_Range_RTE_X with (v:=v) (l:=l) (u:=u); crush;eauto |
  ...
  match goal with
  | [H: compile2_flagged_exp _ _ _ |- _] => specialize (exp_exterior_checks_beq_nil _ _ _ H); crush;eauto
  | [H: compile2_flagged_name _ _ _ |- _] => specialize (name_exterior_checks_beq_nil _ _ _ H); crush;eauto
  end.
  rewrite H1 in H0; crush;eauto.
Qed.
...
- destruct args, args', params';
  ...
  inversion H; clear H; crush;eauto;
  match goal with
  | [H: parameter_mode ?x = In |- _] => apply Copy_Out_Mode_In_X; crush;eauto
  | _ => idtac
  end;
  match goal with
  ...
  | [H1: storeUpdate _ _ ?x _ _,
     H2: compile2_flagged_name _ ?x _,
     H3: compile2_flagged_symbol_table _ _ |- _] => specialize (store_update_checks_soundness _ _ _ _ _ _ _ H1 H2 H3); crush;eauto
  | _ => idtac
  end;
  [ apply Copy_Out_Mode_Out_nRTE with (v:=v); crush;eauto |
    apply Copy_Out_Mode_Out_NoRange_X with (v:=v) (s':=s'); crush;eauto |
    apply Copy_Out_Mode_Out_Range_RTE_X with (v:=v) (t:=t0) (l:=l) (u:=u); crush;eauto |
    apply Copy_Out_Mode_Out_Range_nRTE_X with (v:=v) (t:=t0) (l:=l) (u:=u); crush;eauto |
    apply Copy_Out_Mode_Out_Range_X with (v:=v) (t:=t0) (l:=l) (u:=u) (s':=s'); crush;eauto |
    apply Copy_Out_Mode_Out_nRTE with (v:=v); crush;eauto |
    apply Copy_Out_Mode_Out_NoRange_X with (v:=v) (s':=s'); crush;eauto |
    apply Copy_Out_Mode_Out_nRTE with (v:=v); crush;eauto |
    apply Copy_Out_Mode_Out_NoRange_X with (v:=v) (s':=s'); crush;eauto |
...
Proof.
  ...
  | [H1: fetch_exp_type ?x ?st = Some _,
     H2: fetch_exp_type ?x ?st = Some _ |- _] => rewrite H1 in H2; crush;eauto
  | _ => idtac
  end;
  match goal with
  | [H1: is_range_constrainted_type ?t = false,
  ...
  match goal with
  | [H: compile2_flagged_exp _ _ ?e' |- context[exp_exterior_checks ?e']] => specialize (exp_exterior_checks_beq_nil _ _ _ H); crush;eauto
  | [ |- eval_expr_x _ _ (update_exterior_checks_exp _ _) _] => apply eval_exp_ex_cks_added; auto
  | [ |- context[exp_exterior_checks (update_exterior_checks_exp _ _)]] => rewrite exp_updated_exterior_checks; auto
  | [H: compile2_flagged_name ?st0 _ ?x' |- context[fetch_exp_type_x (name_astnum_x ?x') _]] =>
      rewrite <- (name_ast_num_eq _ _ _ H); apply symbol_table_exp_type_rel with (st := st0); auto
...
- (* S_Null *)
  ...
  constructor.
- (* S_Assignment *)
  apply statement_assign_checks_soundness with (st:=st) (ast_num:=ast_num) (x:=x) (e:=e); crush;eauto.
...
- (* If True *)
  ...
  apply IHeval_stmt; crush;eauto.
- (* If False *)
  inversion H1; clear H1; subst;
  apply Eval_S_If_False_X; crush;eauto.
  match goal with
  | [H1: eval_expr _ ?s ?e ?v,
     H2: compile2_flagged_exp _ ?e ?e',
     H3: compile2_flagged_symbol_table _ _ |- _] => 
...
- (* While RTE *)
  ...
  specialize (expression_checks_soundness _ _ _ _ _ _ H H7 H1); crush;eauto.
- (* While True RTE *)
  inversion H1; clear H1; subst;
  apply Eval_S_While_Loop_True_RTE_X.
  specialize (expression_checks_soundness _ _ _ _ _ _ H H8 H2); crush;eauto.
  apply IHeval_stmt; crush;eauto.
- (* While True *)
  inversion H2; clear H2; subst;
  apply Eval_S_While_Loop_True_X with (s1:=s1).
  specialize (expression_checks_soundness _ _ _ _ _ _ H H9 H3); crush;eauto.
  apply IHeval_stmt1; crush;eauto.  
  apply IHeval_stmt2; crush;eauto.
  constructor; auto.
- (* While False *)
  inversion H0; clear H0; subst;
...
- (* Procedure Call Args RTE *)
  inversion H1; subst;
  rewrite H in H9; crush;eauto;
  specialize (symbol_table_procedure_rel _ _ _ _ _ H2 H); crush;eauto;
  specialize (procedure_components_rel _ _ _ H5); crush;eauto;
  apply Eval_S_Proc_RTE_Args_X with (n:=n0) (pb:=x); crush;eauto;
  specialize (copy_in_checks_soundness _ _ _ _ _ _ _ _ _ H0 H6 H11 H2); crush;eauto.
- (* Procedure Call Decls RTE *)
  inversion H3; subst;
  rewrite H in H11; crush;eauto;
  specialize (symbol_table_procedure_rel _ _ _ _ _ H4 H); crush;eauto;
  specialize (procedure_components_rel _ _ _ H7); crush;eauto;
  apply Eval_S_Proc_RTE_Decl_X with (n:=n0) (pb:=x) (f:=f) (intact_s:=intact_s) (s1:=s1); crush;eauto.
  specialize (copy_in_checks_soundness _ _ _ _ _ _ _ _ _ H0 H8 H13 H4); crush;eauto.
  specialize (declaration_checks_soundness _ _ _ _ _ _ _ H2 H5 H4); crush;eauto.
- (* Procedure Call Body RTE *)
  inversion H4; subst;
  rewrite H in H12; crush;eauto;
  specialize (symbol_table_procedure_rel _ _ _ _ _ H5 H); crush;eauto;
  specialize (procedure_components_rel _ _ _ H8); crush;eauto;
  apply Eval_S_Proc_RTE_Body_X with (n:=n0) (pb:=x) (f:=f) (intact_s:=intact_s) (s1:=s1) (f1:=f1); crush;eauto.
  specialize (copy_in_checks_soundness _ _ _ _ _ _ _ _ _ H0 H9 H14 H5); crush;eauto.
  specialize (declaration_checks_soundness _ _ _ _ _ _ _ H2 H6 H5); crush;eauto.
- (* Procedure Call *)
  inversion H7; subst;
  rewrite H in H15; crush;eauto;
  specialize (symbol_table_procedure_rel _ _ _ _ _ H8 H); crush;eauto;
  specialize (procedure_components_rel _ _ _ H10); crush;eauto;
  specialize (IHeval_stmt _ _ H13 H8); crush;eauto;
  apply Eval_S_Proc_X with (n:=n0) (pb:=x) (f:=f) (intact_s:=intact_s) (s1:=s1) (f1:=f1)
                            ...
                            (s3:=s3); crush;eauto.
  specialize (copy_in_checks_soundness _ _ _ _ _ _ _ _ _ H0 H11 H17 H8); crush;eauto.
  specialize (declaration_checks_soundness _ _ _ _ _ _ _ H2 H4 H8); crush;eauto.
  specialize (copy_out_checks_soundness _ _ _ _ _ _ _ _ _ H6 H11 H17 H8); crush;eauto.
- (* Sequence RTE *)
...
- (* Sequence *)
  ...
  apply Eval_S_Sequence_X with (s1:=s1); crush;eauto.
Qed.
