...
(** * Expression Checks Optimization Completeness *)
Lemma expression_checks_optimization_completeness: forall e st st' s e' e'' eBound v, 
  toExpRT st e e' ->
    toSymTabRT st st' ->
      well_typed_stack st' s ->
        well_typed_exp_x st' e' ->
...
Ltac apply_expression_checks_optimization_completeness :=
  match goal with
  | [H1: toExpRT ?st ?e ?e',
     H2: toSymTabRT ?st ?st',
     H3: well_typed_stack ?st' ?s,
     ...
     H6: evalExpRT ?st' ?s ?e' ?v |- _] => 
      specialize (expression_checks_optimization_completeness _ _ _ _ _ _ _ _ H1 H2 H3 H4 H5 H6);
      let HZ := fresh "HZ" in
      intro HZ
...
(** * Expression Checks Optimization Soundness *)
...
(** * Name Checks Optimization Completeness *)
Lemma name_checks_optimization_completeness: forall n st st' s n' n'' nBound v, 
  toNameRT st n n' ->
    toSymTabRT st st' ->
      well_typed_stack st' s ->
        well_typed_name_x st' n' ->
...
Ltac apply_name_checks_optimization_completeness := 
  match goal with
  | [H1: toNameRT ?st ?n ?n',
     H2: toSymTabRT ?st ?st',
     H3: well_typed_stack ?st' ?s,
     ...
     H5: optName ?st' ?n' _,
     H6: evalNameRT ?st' ?s ?n' ?v |- _] =>
      specialize (name_checks_optimization_completeness _ _ _ _ _ _ _ _ H1 H2 H3 H4 H5 H6);
      let HZ := fresh "HZ" in
      intro HZ
  | [H1: well_typed_stack_and_symboltable ?st' ?s |- _] => 
      inversion H1; subst; apply_well_typed_stack_infer;
...
(** * Name Checks Optimization Soundness *)
...
- (** E_Indexed_Component_X *)
  ...
  + apply EvalIndexedComponentRTE_RTE with (a:=a0); auto.
    ...
    specialize (optimize_exp_ex_cks_stripped _ _ _ _ _ H20); intro HZ1.
    apply eval_exp_ex_cks_added. 
    apply expression_checks_optimization_soundness with (e:=e) (st:=st) 
...
(** * Store Update Optimization Completeness *)

Lemma store_update_optimization_completeness: forall n st st' s s' n' n'' nBound v, 
  toNameRT st n n' ->
    toSymTabRT st st' ->
      well_typed_stack st' s ->
        well_typed_name_x st' n' ->
...
- inversion H; subst;
  ...
  + apply SU_Indexed_Component_xRTE_X; auto.
    apply_name_checks_optimization_completeness; auto.
  + apply SU_Indexed_Component_eRTE_X with (a:=a0); auto.
    destruct H26; apply_name_checks_optimization_completeness; auto.
    apply eval_expr_value_reserve with (e := e') (eBound := Interval u v0) (rBound := Interval u' v'); auto.
    apply_well_typed_exp_preserve.
    specialize (optimize_exp_ex_cks_stripped _ _ _ _ _ H20); intro HZ1.
    specialize (eval_exp_ex_cks_stripped _ _ _ _ _ H27); intros HZ2.
  ...
  + apply_well_typed_exp_preserve.
    ...
    destruct H12; apply_name_checks_optimization_completeness; auto.
    apply eval_expr_value_reserve with (e := e') (eBound := Interval u v0) (rBound := Interval u' v'); auto.
    apply_expression_checks_optimization_completeness.
    specialize (eval_exp_ex_cks_stripped _ _ _ _ _ HZ0); auto.
...
- inversion H; subst;
  ...
  inversion H3; subst;
  inversion H4; subst.
  + apply_name_checks_optimization_completeness;
...
Ltac apply_store_update_optimization_completeness := 
  match goal with
  | [H1: toNameRT ?st ?n ?n' ,
     H2: toSymTabRT ?st ?st' ,
     H3: well_typed_stack ?st' ?s ,
     ...
     H6: storeUpdateRT ?st' ?s ?n' ?v ?s' |- _] =>
      specialize (store_update_optimization_completeness _ _ _ _ _ _ _ _ _ H1 H2 H3 H4 H5 H6);
      let HZ := fresh "HZ" in
      intro HZ
  | [H1: well_typed_stack_and_symboltable ?st' ?s |- _] => 
      inversion H1; subst; apply_well_typed_stack_infer;
...
(** * Store Update Optimization Soundness *)
...
(** * Copy_in_args Checks Optimization Completeness *)
Lemma copy_in_args_checks_optimization_completeness: forall params' st st' s f f' params args args' args'',
...
Ltac apply_copy_in_args_checks_optimization_completeness :=
  match goal with
  | [H1: toArgsRT ?st ?params ?args ?args',
     ...
     H7: copyInRT ?st' ?s ?f ?params' ?args' ?f' |- _ ] =>
      specialize (copy_in_args_checks_optimization_completeness _ _ _ _ _ _ _ _ _ _ H1 H2 H3 H4 H5 H6 H7);
      let HZ := fresh "HZ" in intro HZ
  end.
...
(** * Copy_in_args Checks Optimization Soundness *)
Lemma copy_in_args_checks_optimization_soundness: forall st params args args' st' s f f' params' args'',
  toArgsRT st params args args' ->
  toSymTabRT st st' ->
  toParamSpecsRT params params' ->
  well_typed_stack_and_symboltable st' s ->
...
-(* C2_Flagged_Args_InOut_In_RangeCheck *)
 ...
 apply name_checks_optimization_soundness 
...
Ltac apply_copy_in_args_checks_optimization_soundness' :=
  ...
  | [H1: toArgsRT ?st ?params ?args ?args',
     ...
     H6: optArgs ?st' ?params' ?args' ?args'',
     H7: copyInRT ?st' ?s ?f ?params' ?args'' ?f' |- _ ] =>
      specialize (copy_in_args_checks_optimization_soundness' _ _ _ _ _ _ _ _ _ _ H1 H2 H3 H4 H4b H5 H6 H7);
      let HZ := fresh "HZ" in intro HZ
  end.
