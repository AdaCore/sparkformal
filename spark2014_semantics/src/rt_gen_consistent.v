...
Lemma toExpRT_soundness: forall e e' st st' s v,
  evalExpRT st' s e' v ->
    toExpRT st e e' ->
      toSymTabRT st st' ->
        evalExp st s e v.
...
Ltac apply_toExpRT_soundness := 
  match goal with
  | [H1: evalExpRT ?st' ?s ?e' ?v,
     H2: toExpRT ?st ?e ?e',
     H3: toSymTabRT ?st ?st' |- _] => 
      specialize (toExpRT_soundness _ _ _ _ _ _ H1 H2 H3);
      let HZ := fresh "HZ" in intro HZ
  end.
...
Lemma toNameRT_soundness: forall n n' st st' s v,
  evalNameRT st' s n' v ->
    toNameRT st n n' ->
      toSymTabRT st st' ->
        evalName st s n v.
...
Ltac apply_toNameRT_soundness :=
  match goal with
  | [H1: evalNameRT ?st' ?s ?n' ?v,
     H2: toNameRT ?st ?n ?n',
     H3: toSymTabRT ?st ?st' |- _ ] => 
      specialize (toNameRT_soundness _ _ _ _ _ _ H1 H2 H3);
      let HZ := fresh "HZ" in intro HZ
  end.
...
Lemma toDeclRT_soundness: forall st s f d f' d' st',
...
Ltac apply_toDeclRT_soundness :=
  match goal with
  | [H1: evalDeclRT ?st' ?s ?f ?d' ?f',
     H2: toDeclRT ?st ?d ?d',
     H3: toSymTabRT ?st ?st' |- _] =>
      specialize (toDeclRT_soundness _ _ _ _ _ _ _ H1 H2 H3);
      let HZ := fresh "HZ" in intro HZ
  end.

(** ** storeUpdateRT_soundness *)

Lemma storeUpdateRT_soundness: forall st s x v s' x' st',
...
- match goal with
  ...
  try (apply_eval_exp_ex_cks_stripped);
  try (apply_toNameRT_soundness; smack);
  try (apply_toExpRT_soundness; smack);
  match goal with
...
Ltac apply_storeUpdateRT_soundness :=
  match goal with
  | [H1: storeUpdateRT ?st' ?s ?x' ?v ?s',
     H2: toNameRT ?st ?x ?x',
     H3: toSymTabRT ?st ?st' |- _ ] =>
      specialize (storeUpdateRT_soundness _ _ _ _ _ _ _ H1 H2 H3);
      let HZ := fresh "HZ" in intro HZ
  end.
...
Lemma copyInRT_soundness: forall params st s f args f' params' args' st',
...
Ltac apply_copyInRT_soundness :=
  match goal with
  | [H1: copyInRT ?st' ?s ?f ?params' ?args' ?f',
     H2: toParamSpecsRT ?params ?params',
     H3: toArgsRT ?st ?params ?args ?args',
     H4: toSymTabRT ?st ?st' |- _] => 
      specialize (copyInRT_soundness _ _ _ _ _ _ _ _ _ H1 H2 H3 H4);
      let HZ := fresh "HZ" in intro HZ
  end.
...
Lemma copyOutRT_soundness: forall params st s f args f' params' args' st',
...
- destruct args, args', params';
  ...
  try (apply_storeUpdateRT_soundness);
...
Ltac apply_copyOutRT_soundness :=
  match goal with
  | [H1: copyOutRT ?st' ?s ?f ?params' ?args' ?f',
     H2: toParamSpecsRT ?params ?params',
     H3: toArgsRT ?st ?params ?args ?args',
     H4: toSymTabRT ?st ?st' |- _ ] =>
      specialize (copyOutRT_soundness _ _ _ _ _ _ _ _ _ H1 H2 H3 H4);
      let HZ := fresh "HZ" in intro HZ
  end.


(** ** toAssignStmtRT_soundness *)

Lemma toAssignStmtRT_soundness: forall st s n x e s' stmt' st',
...
*)
  ...
  try (apply_toExpRT_soundness; smack);
  try (apply_storeUpdateRT_soundness; smack);
...
Ltac apply_toAssignStmtRT_soundness :=
  match goal with
  | [H1: evalStmtRT ?st' ?s ?stmt' ?s',
     H2: toStmtRT ?st (Assign ?n ?x ?e) ?stmt',
     H3: toSymTabRT ?st ?st' |- _ ] =>
      specialize (toAssignStmtRT_soundness _ _ _ _ _ _ _ _ H1 H2 H3);
      let HZ := fresh "HZ" in intro HZ
  end.

(** ** toStmtRT_soundness *)

Lemma toStmtRT_soundness: forall st s stmt s' stmt' st',
  evalStmtRT st' s stmt' s' -> 
...
- (* Assign *)
  ...
  [ apply toAssignStmtRT_soundness with (st':=st) (stmt' := (AssignRT n x e)); smack |
    apply toAssignStmtRT_soundness with (st':=st) (stmt' := ((AssignRT n x
...
Ltac apply_toStmtRT_soundness :=
  match goal with
  | [H1: evalStmtRT ?st' ?s ?stmt' ?s',
     H2: toStmtRT ?st ?stmt ?stmt',
     H3: toSymTabRT ?st ?st' |- _ ] =>
      specialize (toStmtRT_soundness _ _ _ _ _ _ H1 H2 H3);
      let HZ := fresh "HZ" in intro HZ
  end.

(** ** toProgramRT_soundness *)

Lemma toProgramRT_soundness: forall st s p s' p' st',
  evalProgramRT st' s p' s' -> 
    toProgramRT st p p' ->
      toSymTabRT st st' ->
        evalProgram st s p s'.
Proof.
  intros.
  destruct p, p'.
  inversion H; inversion H0; subst.
  apply EvalProgram with (n:=n) (pn := pn); auto.
...
Ltac apply_toProgramRT_soundness :=
  match goal with
  | [H1: evalProgramRT ?st' ?s ?p' ?s',
     H2: toProgramRT ?st ?p ?p',
     H3: toSymTabRT ?st ?st' |- _] =>
      specialize (toProgramRT_soundness _ _ _ _ _ _ H1 H2 H3);
      let HZ := fresh "HZ" in intro HZ    
  end.
...
(** ** toExpRT_completeness *)

Lemma toExpRT_completeness: forall e e' st st' s v,
  evalExp st s e v ->
    toExpRT st e e' ->
      toSymTabRT st st' ->
        evalExpRT st' s e' v.
...
Ltac apply_toExpRT_completeness :=
  match goal with
  | [H1: evalExp ?st ?s ?e ?v,
     H2: toExpRT ?st ?e ?e',
     H3: toSymTabRT ?st ?st' |- _ ] => 
      specialize (toExpRT_completeness _ _ _ _ _ _ H1 H2 H3);
      let HZ := fresh "HZ" in intro HZ
  end.


(** ** toNameRT_completeness *)
Lemma toNameRT_completeness: forall n n' st st' s v,
  evalName st s n v ->
    toNameRT st n n' ->
      toSymTabRT st st' ->
        evalNameRT st' s n' v.
Proof.
  ...
  try (apply_toExpRT_completeness; smack);
  match goal with
  | [H1: forall (n' : nameRT) (st : symTab) 
        (st' : symTabRT) (s : STACK.state) (v : Ret value),
      evalName _ _ ?n _ ->
...
Ltac apply_toNameRT_completeness :=
  match goal with
  | [H1: evalName ?st ?s ?n ?v, 
     H2: toNameRT ?st ?n ?n',
     H3: toSymTabRT ?st ?st' |- _] => 
      specialize (toNameRT_completeness _ _ _ _ _ _ H1 H2 H3);
      let HZ := fresh "HZ" in intro HZ
  end.


(** ** toDeclRT_completeness *)

Lemma toDeclRT_completeness: forall st s f d f' d' st',
  evalDecl st s f d f' ->
    toDeclRT st d d' ->
      toSymTabRT st st' ->
        evalDeclRT st' s f d' f'.
Proof.
  ...
  try (apply_toExpRT_completeness);
...
(** ** storeUpdateRT_completeness *)

Lemma storeUpdateRT_completeness: forall st s x v s' x' st',
  storeUpdate st s x v s' ->
    toNameRT st x x' ->
      toSymTabRT st st' ->
        storeUpdateRT st' s x' v s'.
...
- match goal with
  ...
  try (apply_toNameRT_completeness; smack);
  try (apply_toExpRT_completeness; smack);
  match goal with
...
- match goal with
  ...
  | [H1: evalName _ ?s ?n ?v,
     H2: toNameRT _ ?n ?n',
     H3: toSymTabRT _ _ |- _] => specialize (toNameRT_completeness _ _ _ _ _ _ H1 H2 H3); smack
  | _ => idtac
  end;
  match goal with
  | [H1: storeUpdate _ _ ?x _ _ ,
...
Ltac apply_storeUpdateRT_completeness :=
  match goal with
  | [H1: storeUpdate ?st ?s ?x ?v ?s',
     H2: toNameRT ?st ?x ?x',
     H3: toSymTabRT ?st ?st' |- _] =>
      specialize (storeUpdateRT_completeness _ _ _ _ _ _ _ H1 H2 H3);
      let HZ := fresh "HZ" in intro HZ
  end.


(** ** copyInRT_completeness *)

Lemma copyInRT_completeness: forall params st s f args f' params' args' st',
  copyIn st s f params args f' ->
    toParamSpecsRT params params' ->
      toArgsRT st params args args' ->
        toSymTabRT st st' ->
...
Ltac apply_copyInRT_completeness :=
  match goal with
  | [H1: copyIn ?st ?s ?f ?params ?args ?f',
     H2: toParamSpecsRT ?params ?params',
     H3: toArgsRT ?st ?params ?args ?args',
     H4: toSymTabRT ?st ?st' |- _] =>
      specialize (copyInRT_completeness _ _ _ _ _ _ _ _ _ H1 H2 H3 H4);
      let HZ := fresh "HZ" in intro HZ
  end.


(** ** copyOutRT_completeness *)

Lemma copyOutRT_completeness: forall params st s f args f' params' args' st',
  copyOut st s f params args f' ->
    toParamSpecsRT params params' ->
      toArgsRT st params args args' ->
        toSymTabRT st st' ->
...
- destruct args, args', params';
  ...
  try (apply_storeUpdateRT_completeness);
...
Ltac apply_copyOutRT_completeness :=
  match goal with
  | [H1: copyOut ?st ?s ?f ?params ?args ?f',
     H2: toParamSpecsRT ?params ?params',
     H3: toArgsRT ?st ?params ?args ?args',
     H4: toSymTabRT ?st ?st' |- _] =>
      specialize (copyOutRT_completeness _ _ _ _ _ _ _ _ _ H1 H2 H3 H4);
      let HZ := fresh "HZ" in intro HZ
  end.

  
(** ** toAssignStmtRT_completeness *)

Lemma toAssignStmtRT_completeness: forall st s n x e s' stmt' st',
  evalStmt st s (Assign n x e) s' -> 
    toStmtRT st (Assign n x e) stmt' ->
      toSymTabRT st st' ->
        evalStmtRT st' s stmt' s'.
Proof.
  ...
  | [H1: evalExp _ ?s ?e ?v,
     H2: toExpRT _ ?e ?e',
     H3: toSymTabRT _ _ |- _] => specialize (toExpRT_completeness _ _ _ _ _ _ H1 H2 H3); smack
  end;
  match goal with
  | [H1: storeUpdate _ _ ?x _ _,
     H2: toNameRT _ ?x _,
     H3: toSymTabRT _ _ |- _] => specialize (storeUpdateRT_completeness _ _ _ _ _ _ _ H1 H2 H3); smack
  | _ => idtac
  end;
  [ apply EvalAssignRT_RTE; auto |
    apply EvalAssignRT with (v:=v); auto |
...
Ltac apply_toAssignStmtRT_completeness :=
  match goal with
  | [H1: evalStmt ?st ?s (Assign ?n ?x ?e) ?s',
     H2: toStmtRT ?st (Assign ?n ?x ?e) ?stmt',
     H3: toSymTabRT ?st ?st' |- _] =>
      specialize (toAssignStmtRT_completeness _ _ _ _ _ _ _ _ H1 H2 H3);
      let HZ := fresh "HZ" in intro HZ
  end.


(** ** toStmtRT_completeness *)

Lemma toStmtRT_completeness: forall st s stmt s' stmt' st',
  evalStmt st s stmt s' -> 
    toStmtRT st stmt stmt' ->
      toSymTabRT st st' ->
        evalStmtRT st' s stmt' s'.
...
- (* S_Null *)
  ...
  constructor.
- (* Assign *)
  apply toAssignStmtRT_completeness with (st:=st) (n:=n) (x:=x) (e:=e); smack.
  apply EvalAssign_RTE; auto.
- apply toAssignStmtRT_completeness with (st:=st) (n:=n) (x:=x) (e:=e); smack.
  apply EvalAssign with (v:=v) (t:=t); auto.
- apply toAssignStmtRT_completeness with (st:=st) (n:=n) (x:=x) (e:=e); smack.
...
- (* If RTE *)
  ...
  | [H1: evalExp _ ?s ?e ?v,
     H2: toExpRT _ ?e ?e',
     H3: toSymTabRT _ _ |- _] => 
      specialize (toExpRT_completeness _ _ _ _ _ _ H1 H2 H3); smack
  end. 
- (* If True *)
  inversion H1; clear H1; subst;
  apply EvalIfRT_True.
...
- (* While RTE *)
  ...
  specialize (toExpRT_completeness _ _ _ _ _ _ H H7 H1); smack.
- (* While True RTE *)
  inversion H1; clear H1; subst;
  apply EvalWhileRT_True_RTE.
  specialize (toExpRT_completeness _ _ _ _ _ _ H H8 H2); smack.
  apply IHevalStmt; smack.
- (* While True *)
  inversion H2; clear H2; subst;
  apply EvalWhileRT_True with (s1:=s1).
  specialize (toExpRT_completeness _ _ _ _ _ _ H H9 H3); smack.
...
- (* Procedure Call *)
  ...
  specialize (toDeclRT_completeness _ _ _ _ _ _ _ H2 H4 H8); smack.
  specialize (copyOutRT_completeness _ _ _ _ _ _ _ _ _ H6 H11 H17 H8); smack.
- (* Sequence RTE *)
  inversion H0; smack;
  apply EvalSeqRT_RTE;
  apply IHevalStmt; smack.
...
Ltac apply_toStmtRT_completeness :=
  match goal with
  | [H1: evalStmt ?st ?s ?stmt ?s', 
     H2: toStmtRT ?st ?stmt ?stmt',
     H3: toSymTabRT ?st ?st' |- _ ] => 
      specialize (toStmtRT_completeness _ _ _ _ _ _ H1 H2 H3);
      let HZ := fresh "HZ" in intro HZ
  end. 


(** ** toProgramRT_completeness *)

Lemma toProgramRT_completeness: forall st s p s' p' st',
  evalProgram st s p s' -> 
    toProgramRT st p p' ->
      toSymTabRT st st' ->
        evalProgramRT st' s p' s'.
Proof.
  ...
  apply toStmtRT_completeness with (stmt:=(Call n pn (main {| decls := decls; main := mainRT |}) nil)) (st:=st); smack.
  inversion H2; subst;
  match goal with
  | [H1: toSymTabRT _ _, H2: fetch_proc _ _ = Some _ |- _ ] =>
      specialize (symbol_table_procedure_rel _ _ _ _ _ H1 H2); smack
  end;
  match goal with
  | [H: fetch_proc ?p ?st = Some (?n1, ?p1) |- _] => 
...
Ltac apply_toProgramRT_completeness :=
  match goal with
  | [H1: evalProgram ?st ?s ?p ?s',
     H2: toProgramRT ?st ?p ?p',
     H3: toSymTabRT ?st ?st' |- _] =>
      specialize (toProgramRT_completeness _ _ _ _ _ _ H1 H2 H3);
      let HZ := fresh "HZ" in intro HZ
  end.
...
Proof.
  ...
  split; intro;
  [apply_toExpRT_soundness; auto |
   apply_toExpRT_completeness; auto 
  ].
Qed.
...
Proof.
  intros;
  split; intro;
  [apply_toNameRT_soundness; auto |
   apply_toNameRT_completeness; auto 
  ].
Qed.
...
Proof.
  intros;
  split; intro;
  [eapply toDeclRT_soundness; smack |
   eapply toDeclRT_completeness; smack
  ].
Qed.
...
Proof.
  intros;
  split; intro;
  [apply_toStmtRT_soundness; auto |
   apply_toStmtRT_completeness; auto
  ].
Qed.

(** ** toProgramRTConsistent  *)

Lemma toProgramRTConsistent: forall st s p s' p' stRT,
  toProgramRT st p p' ->
    toSymTabRT st stRT ->
      (evalProgramRT stRT s p' s' <-> evalProgram st s p s').
Proof.
  intros;
  split; intro;
  [apply_toProgramRT_soundness; auto |
   apply_toProgramRT_completeness; auto
  ].
Qed.
